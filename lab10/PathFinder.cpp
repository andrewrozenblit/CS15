...
{
    //std::cout << "BFS would run... TODO: needs implementing!" << std::endl;
    std::queue<City> bfs; 
    bfs.push(graph.get_vertex(c1));
    while (bfs.front().get_name() != c2) {
        if (!graph.is_marked(bfs.front())) {
            graph.get_to_vertices(bfs.front(),bfs);
            graph.mark_vertex(bfs.front());
        }
        bfs.pop();
        if (bfs.empty()) {
            return false; 
...
{
    std::stack<City> dfs; 
    dfs.push(graph.get_vertex(c1));
    while (dfs.top().get_name() != c2) {
        if (!graph.is_marked(dfs.top())) {
            graph.get_to_vertices(dfs.top(),bfs);
            graph.mark_vertex(dfs.top());
        }
        dfs.pop();
        if (dfs.empty()) {
            return false; 
