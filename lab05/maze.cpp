...
bool Maze::solve(Position p)
{
    ...
    else if (maze[p.row][p.col] == 'X' || maze[p.row][p.col] == 'b' || 
             maze[p.row][p.col] == '.') {
            return false; 
    }
    ...
    if (solve(p.north()) || solve(p.east()) || solve(p.south()) || solve(p.west())) {
        return true;
    }
...
int Maze::num_dots_by_rows(int row)
{
    ...
    int length = num_dots_in_row(row,0);
    return length + num_dots_by_rows(row+1);
...
int Maze::num_dots_in_row(int row, int col)
{
    if (col == maze_array_size) {
        ...
        return num_dots_in_row(row,col+1) + 1;
    } else {
